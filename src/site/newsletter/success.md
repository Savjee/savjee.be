    ...
    <div class="mt-12 not-prose">
        ...
        <div class="flex flex-col">
            ...
            {% for issue in recent_issues %}
                {% if forloop.index <= 5 %}
                    <div class="mb-10">
                        <span class="text-sm opacity-60 block">Issue {{ issue.data.title | split: ":" | first }} &middot; {{ issue.date | date: "%B %Y" }}</span>
                        <a href="{{ issue.url }}" class="font-bold text-xl text-savjeered hover:underline leading-snug">{{ issue.data.title | split: ":" | last | strip }}</a>
                    </div>
                {% endif %}
            {% endfor %}
        </div>
        ...
        <div class="mt-4">
            <a href="/newsletter/" class="font-bold underline text-savjeered">View all past editions &rarr;</a>
        </div>
    </div>
