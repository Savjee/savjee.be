    ...
    <div class="mt-12">
        ...
        <div class="space-y-6 not-prose">
            ...
            {% for issue in recent_issues %}
                {% if forloop.index <= 5 %}
                    <div class="border-l-4 border-savjeered pl-4">
                        <span class="text-sm opacity-60 block mb-1">Issue {{ issue.data.title | split: ":" | first }} &middot; {{ issue.date | date: "%B %Y" }}</span>
                        <a href="{{ issue.url }}" class="font-bold text-lg text-savjeered hover:underline decoration-2 underline-offset-4">{{ issue.data.title | split: ":" | last | strip }}</a>
                    </div>
                {% endif %}
            {% endfor %}
        </div>
